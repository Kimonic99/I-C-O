...
export default function Home() {
  ...
  const mintCryptoDevToken = async (amount) => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Successfully minted Naija Dev Tokens");
  ...
  const claimCryptoDevTokens = async () => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Sucessfully claimed Naija Dev Tokens");
      await getBalanceOfCryptoDevTokens();
      await getTotalTokensMinted();
      await getTokensToBeClaimed();
    } catch (err) {
  ...
  return (
    <div>
      <Head>
        <title>Naija Devs</title>
        <meta name="description" content="ICO-Dapp" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Welcome to Naija Devs ICO!</h1>
          <div className={styles.description}>
            You can claim or mint Naija Dev tokens here
          </div>
          {walletConnected ? (
            <div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
                You have minted {utils.formatEther(balanceOfCryptoDevTokens)} Naija
                Dev Tokens
              </div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
